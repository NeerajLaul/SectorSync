...
export function SignUpPage({ onSignUp, onSignIn, onSkip }: SignUpPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("https://sectorsync-production.up.railway.app/api/auth/signup", {
        method: "POST",
      ...
      // If success, user is logged in (cookie set)
      onSignUp({ fullName, email, company, password }); // Pass data up to parent
    } catch (err: any) {
      console.error(err);
      // You might want to add an error state to display to the user
      alert(err.message);
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  required
                  className={`bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm border-white/20 dark:border-white/10 pr-10 ${!passwordMatch ? "border-red-500" : ""
                    }`}
                />
                <button
                  type="button"
